"""
scripts/run_path_ad_verdict.py — Path AD v3 alpha class verdict.

Top-1500 cumulative momentum (12-1 skip 4w) per docs/spec_path_ad_*_v1.md.

5 v3 alpha gates:
  G1 Sharpe (net 10bp TC, ann.) >= 0.30
  G2 Newey-West HAC t-stat (lag-8) > 1.96
  G3 max |rho| vs K1/D-PEAD/PATH_N/CTA weekly <= 0.25
  G5-v2 DD attenuation >= 2/3 crisis windows
  G6 Portfolio Sharpe lift at any w in {2,5,10,15,20}% >= 2%
...
def main() -> None:
    print("=" * 78)
    print("Path AD Cross-Sectional Momentum on top-1500 — v3 ALPHA class verdict")
    print("=" * 78)
    print()

    pv_path = REPO_ROOT / "data" / "portfolio_replay" / "v1_path_ad_top1500_momentum_weekly.parquet"
    pv = pd.read_parquet(pv_path)
    pv.index = pd.to_datetime(pv.index)
    pv_net = pv["net"].dropna()
    print(f"Path AD net returns: n={len(pv_net)} weeks")
    ...
    for label, (start, end) in CRISIS_WINDOWS.items():
        ...
        print(f"               [{marker}] {label}: AD DD = {cand_dd*100:+.2f}%, SPY DD = {spy_dd*100:+.2f}%")
    g5_pass = g5_wins >= 2
    print(f"             G5-v2 result: {g5_wins} of 3   (>= 2/3)   -> {'PASS' if g5_pass else 'FAIL'}")

    # G6 portfolio Sharpe lift
    print()
    ...
    print("=" * 78)
    print(f"  Path AD: {n_pass}/5 v3 alpha gates PASS  ->  VERDICT: {verdict}")
    print("=" * 78)

    today = datetime.date.today()
    payload = {
        "spec_id":   78,
        "spec_hash": "2cd3bd92025aa0ab66442142028474c911a9f990",
        "spec_name": "Cross-Sectional Cumulative Momentum on top-1500 (J-T 1993 on broader universe)",
    ...
    out_path = REPO_ROOT / "data" / "portfolio_replay" / f"path_ad_verdict_{today.isoformat()}.json"
